...
function render () {
	...
	output.push(textTable(runners, {
		align: ['l', 'l', 'l'],
		stringLength: str => chalk.reset(str).length,
	}));
	...
	while (i--) {
		...
		switch (type) {
			default:
			case 'message':
				m += chalk.bold.blue('○ ' + RUNNER_NAMES[key]);
				break;
			case 'warning':
				m += chalk.bold.yellow('! ' + RUNNER_NAMES[key]);
				break;
			case 'error':
				m += chalk.bold.red('✘ ' + RUNNER_NAMES[key]);
				break;
		}

		m += chalk.grey(' :: ');

		// Notice: remember to chalk.reset the string before crop-/wrapping
		if (/\r\n|\r|\n/.exec(message)) {
			m += '\n' + message; // TODO: contain within columns by cropping
		} else {
			m += message; // TODO: contain within columns by wrapping
		}

		availableRows -= m.split(/\r\n|\r|\n/).length;

		// FIXME: This isn't working 100% of the time, and seems to be adding
		//  the length of the last line in spaces to the first line?
		if (availableRows < 0) {
			const lines = m.split(/\r\n|\r|\n/);
			lines.splice(-1, availableRows * -1);
			m = lines.join('\n');
	...
	// Redraw the console
	process.stdout.write('\033[2J' + output.join('\n'));
}
