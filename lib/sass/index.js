...
async function compile (entry) {
	...
	try {
		...
		// Update the manifest
		manifest(entry.name, path.basename(output));
 ...
 * @param {Array} imports
 */
function updateWatcher (entry, imports) {
	if (!watchers[entry.path])
		return;

	const { watcher, imports: oldImports } = watchers[entry.path];
