...
const urls = Object.keys(config.critical.paths).map(tmpl => ({
	template: tmpl,
	url: `${config.critical.baseUrl}/${config.critical.paths[template]}`
		.replace('//', '/')
		.replace(':/', '://'),
}));
...
async function startJob () {
	...
	try {
		// Get CSS string
		const targetMarkup = getUrl(url);
		const cssString = getCss(targetMarkup);
...
async function getCss (markup) {
	...
	for (let i = 0, l = urls.length; i < l; i++) {
		...
		if (url.indexOf('[') > -1)
			url.replace(FILE_RX, (_, key) => manifest.read(key));
